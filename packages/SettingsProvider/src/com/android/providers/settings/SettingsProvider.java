...
public class SettingsProvider extends ContentProvider {
    ...
    private static final SparseArray<SettingsCache> sSecureCaches
            = new SparseArray<SettingsCache>();
   private static final SparseArray<SettingsCache> sCMREMIXCaches
            = new SparseArray<SettingsCache>();
        ...
        SqlArguments(Uri url, String where, String[] args) {
            if (url.getPathSegments().size() == 1) {
                ...
                if (TABLE_SYSTEM.equals(this.table) || TABLE_SECURE.equals(this.table) ||
                    TABLE_GLOBAL.equals(this.table) || TABLE_CMREMIX.equals(this.table)) {
    ...
    private void sendNotify(Uri uri, int userHandle) {
        ...
        if (table.equals(TABLE_SYSTEM)) {
            ...
            backedUpDataChanged = true;
        } else if (table.equals(TABLE_CMREMIX)) {
            property = Settings.CMREMIX.SYS_PROP_SETTING_VERSION;
            backedUpDataChanged = true;
        } else if (isGlobal) {
    ...
    void onUserRemoved(int userHandle) {
        synchronized (this) {
            ...
            sCMREMIXCaches.delete(userHandle);
    ...
    private void establishDbTracking(int userHandle) {
        ...
        synchronized (this) {
            ...
            if (dbhelper == null) {
                ...
                sCMREMIXCaches.append(userHandle, new SettingsCache(TABLE_CMREMIX));
    ...
    private void fullyPopulateCaches(final int userHandle) {
        ...
        fullyPopulateCache(dbHelper, TABLE_CMREMIX, sCMREMIXCaches.get(userHandle));
    }
    ...
    public SettingsCache cacheForTable(final int callingUser, String tableName) {
        ...
        if (TABLE_CMREMIX.equals(tableName)) {
            return getOrConstructCache(callingUser, sCMREMIXCaches);
        }
    ...
    public Bundle call(String method, String request, Bundle args) {
        ...
        if (Settings.CALL_METHOD_GET_CMREMIX.equals(method)) {
            if (LOCAL_LOGV) Slog.v(TAG, "call(cmremix:" + request + ") for " + callingUser);
            dbHelper = getOrEstablishDatabase(callingUser);
            cache = sCMREMIXCaches.get(callingUser);
            return lookupValue(dbHelper, TABLE_CMREMIX, cache, request);
        }
        ...
        } else if (Settings.CALL_METHOD_PUT_CMREMIX.equals(method)) {
            if (LOCAL_LOGV) Slog.v(TAG, "call_put(cmremix:" + request + "=" + newValue + ") for " + callingUser);
            insertForUser(Settings.CMREMIX.CONTENT_URI, values, callingUser);
        } else {
